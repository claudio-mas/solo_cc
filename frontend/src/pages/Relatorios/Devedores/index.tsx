/**
 * pages/Relatorios/Devedores/index.tsx — Equivalente web de frmRptDevedores1
 * Sistema: Solo Consultoria de Imóveis — Contas Correntes
 *
 * Regras implementadas:
 *   RN108 — Retorna clientes com SUM(Deb) > SUM(Cred) até a data de corte
 *   RN109 — Filtro por faixa de código
 *   RN110 — Filtro por saldo mínimo
 *   RN111 — Ordenação por código ou nome
 *   RN112 — Título dinâmico com data e saldo mínimo
 *   RN113 — Saldo colorido (devedor)
 *   RN107 — Parâmetros preservados ao voltar (ADAPTAR — React Router state)
 *
 * B67 — PRESERVAR (query de devedores)
 * B71 — DESCARTAR (C1Report + C1PrintPreviewControl)
 * B72 — DESCARTAR (frmReports.Visible)
 * B73 — ADAPTAR (tabela simples em vez de GroupBy do C1Report)
 */
      ...
      <Page size="A4" style={pdfStyles.page}>
        ...
        {itens.map((item) => (
          <View key={item.id} style={pdfStyles.row}>
            ...
            <Text style={[pdfStyles.colSaldo, pdfStyles.saldoDevedor]}>
              ...
              <tbody>
                {itens.map((item) => (
                  <tr key={item.id}>
                    ...
                    {/* RN113 — saldo devedor em vermelho */}
                    <td className={`${styles.tdSaldo} ${styles.saldoDevedor}`}>
                      {formatarMoeda(item.saldo)}
                    </td>
                  </tr>
                ))}
